...
def compute_synergy_loss(
    results, mean_control, high_umi_idx, subtype="POTENTIATION"
):
    pred_res = get_test_set_results_seen2(results, subtype)
    all_perts = np.unique(results["pert_cat"])
    linear_params = get_linear_params(
        pred_res, high_umi_idx, mean_control, all_perts
    )
    synergy_loss = np.sum(
        [
            np.abs(
                linear_params[k]["pred"]["mag"]
                - linear_params[k]["truth"]["mag"]
            )
    ...
    mag = np.sum(
        [np.abs(linear_params[k]["pred"]["mag"]) for k in linear_params]
    )
...
def non_zero_analysis(adata, test_res):
    ...
    geneid2idx = dict(
        zip(adata.var.index.values, range(len(adata.var.index.values)))
    )
    ...
    for pert in np.unique(test_res["pert_cat"]):
        ...
        direc_change = np.abs(
            np.sign(
                test_res["pred"][pert_idx].mean(0)[de_idx] - ctrl[0][de_idx]
            )
            - np.sign(
        ...
        pert_metric[pert][
            "frac_0/1_direction_top20_non_zero"
        ] = frac_direction_opposite
        ...
        zero_des = np.intersect1d(
            np.where(min_ == 0)[0], np.where(max_ == 0)[0]
        )
        ...
        if len(nonzero_des) == 0:
            ...
            pred_mean = np.mean(
                test_res["pred"][pert_idx][:, de_idx], axis=0
            ).reshape(
                -1,
            )
            # true_mean = np.mean(
            #     test_res["truth"][pert_idx][:, de_idx], axis=0
            # ).reshape(
            #     -1,
            # )
            ...
            pert_metric[pert][
                "frac_in_range_25_75_non_zero"
            ] = frac_in_range_25_75

            zero_idx = np.where(std > 0)[0]
            sigma = (np.abs(pred_mean[zero_idx] - mean[zero_idx])) / (
                std[zero_idx]
            )
...
def non_dropout_analysis(adata, test_res):
    ...
    for pert in np.unique(test_res["pert_cat"]):
        ...
        de_idx = [
            geneid2idx[i]
            for i in adata.uns["top_non_dropout_de_20"][
        ...
        direc_change = np.abs(
            np.sign(
                test_res["pred"][pert_idx].mean(0)[non_zero_idx]
                - ctrl[0][non_zero_idx]
            )
            - np.sign(
        ...
        pert_metric[pert][
            "frac_opposite_direction_non_zero"
        ] = frac_direction_opposite
        ...
        pert_metric[pert][
            "frac_0/1_direction_non_dropout"
        ] = frac_direction_opposite
        ...
        if len(nonzero_des) == 0:
            ...
            frac_in_range_45_55 = sum(in_range_5) / len(nonzero_des)
            pert_metric[pert][
                "frac_in_range_45_55_non_dropout"
            ] = frac_in_range_45_55
            ...
            frac_in_range_40_60 = sum(in_range_10) / len(nonzero_des)
            pert_metric[pert][
                "frac_in_range_40_60_non_dropout"
            ] = frac_in_range_40_60
            ...
            frac_in_range_25_75 = sum(in_range_25) / len(nonzero_des)
            pert_metric[pert][
                "frac_in_range_25_75_non_dropout"
            ] = frac_in_range_25_75
...
def deeper_analysis(
    adata,
    test_res,
    de_column_prefix="rank_genes_groups_cov",
    ...
    for pert in np.unique(test_res["pert_cat"]):
        ...
        de_idx = [
            geneid2idx[i]
            for i in adata.uns["rank_genes_groups_cov_all"][
                pert2pert_full_id[pert]
            ][:20]
        ]
        de_idx_200 = [
        ...
        ## this is to remove the ones that are super low and the fold change
        # becomes unmeaningful
        ...
        o = np.intersect1d(
            np.where(fold_change[0] < 0.333)[0],
            np.where(fold_change[0] > 0)[0],
        )
...
def get_mean_ctrl(adata):
    return (
        adata[adata.obs["condition"] == "ctrl"]
        .to_df()
        .mean()
...
def get_test_set_results_seen2(res, sel_GI_type):
    ...
    test_pert_cats = [
        p
        for p in np.unique(res["pert_cat"])
        if p in GIs[sel_GI_type] or "ctrl" in p
    ]
...
def get_all_vectors(
    all_res, mean_control, double, single1, single2, high_umi_idx
):
    # Pred
